...
const renderDisplay = () => {
	...
	if (redlineTime >= startingTime && redlineTime <= endingTime)
		canvasContext.fillRect((redlineTime - startingTime) / msPerPixel + KEY_W, HEADER_H, 1.5, CANVAS_H - HEADER_H);
};
...
const renderTrack = () => {
	...
	for (let i = 0; i < notes.length; i++) {
		...
		if (noteTime + noteDuration < startingTime) continue;
			...
			if (reverseScale) {
				if (
					notes[i].midi >= heptatonicScaleNames.length + 21 - KEYS_SHOWN - startingScale &&
					notes[i].midi < heptatonicScaleNames.length + 21 - startingScale
				) {
					let x = noteTime <= startingTime ? KEY_W + 1 : KEY_W + (noteTime - startingTime) / msPerPixel;
					let width;
					if (noteTime <= startingTime)
						width =
							Math.round((noteDuration - (startingTime - noteTime)) / msPerPixel) > CANVAS_W - x
								? CANVAS_W - x
								: Math.round((noteDuration - (startingTime - noteTime)) / msPerPixel);
					else
						width =
					...
					canvasContext.fillRect(
						x,
						HEADER_H + TRACK_H * (heptatonicScaleNames.length + 20 - notes[i].midi - startingScale),
						width,
						TRACK_H
					);
				} else continue;
			} else {
				if (notes[i].midi >= startingScale + 21 && notes[i].midi < startingScale + 21 + KEYS_SHOWN) {
					let x = noteTime <= startingTime ? KEY_W + 1 : KEY_W + (noteTime - startingTime) / msPerPixel;
					let width;
					if (noteTime <= startingTime)
						width =
						...
						width =
							Math.round(noteDuration / msPerPixel) > CANVAS_W - x
								? CANVAS_W - x
								: Math.round(noteDuration / msPerPixel);
					canvasContext.fillRect(x, HEADER_H + TRACK_H * (notes[i].midi - 21 - startingScale), width, TRACK_H);
				} else continue;
			}
		}
	}
